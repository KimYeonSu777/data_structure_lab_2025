...
public:
	...
	// 음악 찾아주는 서비스(Artist) <- Title과의 차이점: Artist 는 반환이 여러개 Title은 한개
	vector<Music*> searchByAltist(string artist) {
		vector<Music*> result;

		for (int i = 0; i < music_list.size(); i++) {
			if (music_list[i].getArtist() == artist) {
				result.push_back(&music_list[i]);            // &music_list[i]로 쓴 이유: result가 		vector<Music*> result; 로 포인터 변수로 선언되어 있기 때문에    // == music_list + i 로 써도 똑같음.
			}
		}
